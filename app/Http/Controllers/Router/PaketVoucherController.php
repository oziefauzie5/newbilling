...
class PaketVoucherController extends Controller
{
    public function store(Request $request)
    {
        ...
        $record = '; :local mac $"mac-address"; :local time [/system clock get time ]; :local wexp [/ip hotspot user get [/ip hotspot user find where name="$user"] comment]; /system script add name="$date-|-$time-|-$user-|-$mac-|-$wexp" owner="$month$year" source=$date comment=fisnet;';
        ...
        if ($data['paket_mode'] == "rem") {
            ...
            $onlogin = $onlogin . $record . $lock .$post. "}}";
            $mode = "remove";
    ...
    public function update_paket_voucher($request, $id)
    {
        ...
        $data_paket = Paket::where('paket_id', $id)->first();
        $API = new RouterosAPI();
        $API->debug = false;
            ...
            if ($API->connect($router->router_ip . ':' . $router->router_port_api, $router->router_username, $router->router_password)) {
                $cek = $API->comm('/ip/hotspot/user/profile/print', [
                    '?name' => $data_paket->paket_nama,
                ]);
                // dd($cek_sch['0']);
                if($cek){
                    $API->comm('/ip/hotspot/user/profile/set', [
                        ...
                        'add-mac-cookie' => 'yes' == '' ? '' : 'yes',
                        'mac-cookie-timeout' => $data['paket_masa_aktif'] == '' ? '' : $data['paket_masa_aktif'],
                        'transparent-proxy' => 'true' == '' ? '' : 'true',
                        
                    ]);
                    
                    
                    if ($data['paket_mode'] != "0") {
                        $cek_sch = $API->comm("/system/scheduler/print", [
                                        "?name" => "$data_paket->paket_nama"
                                    ]);
                            // dd($cek_sch[0]['.id']);
                            $idmiksch = $cek_sch[0]['.id'];
                        if (empty($idmiksch)) {
                            $API->comm("/system/scheduler/add", array(
                                "name" => "$name",
                                "start-time" => "$randstarttime",
